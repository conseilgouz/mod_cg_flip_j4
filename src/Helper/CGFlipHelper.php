...
class CGFlipHelper
{
    ...
    public static function getFlipArticles($params, $moduleid)
        ...
        foreach ($sectionsList as $item) {
            if ($item->sf_type == "category") { // one category selected
                ...
                foreach ($articles as $article) {
                    ...
                    if ($multi) {
                        if ($multicol) {
                            $unepage .= '<div class="cg_un_article fg-c'.(12 / $item->articlecol).'">'.$art.'</div>';
                        } else {
                            $unepage .= '<div class="cg_un_article">'.$art.'</div>';
                        }
                        $col += 1;
                        $nbart += 1;
                    } else {
    ...
    public static function getFlipToc()
    {
        return self::$toc;
    }
    ...
    public static function getCategory($id, $params, $ordering)
    {
        // Get an instance of the generic articles model
    ...
    public static function render_article($article, $params, $multi)
    {
    ...
    public static function linktopage($moduleid, $text)
    {
        $regex = '/\{(?:page)\b(.*)\}/siU';
    ...
    public static function lookfortitle($text)
    {
        $regex = '`(<h2([^>]*)>)(.+?)(?=</h2>)`';
    ...
    //-------------------------------------------- 1.2.0 : Events	----------------------------------
    public static function getFlipEvents($params)
    {
